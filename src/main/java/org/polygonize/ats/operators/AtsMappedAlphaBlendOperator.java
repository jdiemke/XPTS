...
@AtsOperatorMetadata(operatorDesignation = "blend 2", author = "Johannes Diemke", version = "1.0", description = "This operator generates a XOR texture.", date = "2008-08-08", sourceOfSupply = "http://informatik.uni-oldenburg.de/~trigger")

public class AtsMappedAlphaBlendOperator extends AtsProcessOperator {
    ...
    public void process() {
        ...
        AtsProceduralTexture source3 = inputs_.get(2).getTexture();

        float scale = alpha_.value / 255f;

        for (int y = 0; y < 256; y++)
            for (int x = 0; x < 256; x++) {

                float alpha = scale * 2 * (source3.getPixelIntensity(x, y) / 255f);
                if (alpha > 1)
                    alpha = 1;
